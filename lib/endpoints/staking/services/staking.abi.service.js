...
let AbiStakingService = AbiStakingService_1 = class AbiStakingService {
    ...
    async getGroupsByOwner(address) {
        var _a, _b;
        const contract = await this.elrondProxy.getRouterSmartContract();
        const interaction = contract.methodsExplicit.getGroupsByOwner([
            new out_1.AddressValue(new out_1.Address(address)),
        ]);
        const response = await this.getGenericData(contract, interaction);
        return (_b = (_a = response.firstValue) === null || _a === void 0 ? void 0 : _a.valueOf().toString()) !== null && _b !== void 0 ? _b : [];
    }
    ...
    async getRewardTokenIdByGroupId(groupId) {
        var _a;
        const contract = await this.elrondProxy.getRouterSmartContract();
        const interaction = contract.methodsExplicit.getRewardTokenIdByGroupIdentifier([
            new out_1.BytesValue(Buffer.from(groupId)),
        ]);
        const response = await this.getGenericData(contract, interaction);
