...
let StakingGetterService = StakingGetterService_1 = class StakingGetterService {
    ...
    async getData(cacheKey, createValueFunc, ttl = erdnest_1.Constants.oneHour()) {
        var _a, _b;
        try {
            const noCache = (_b = (_a = erdnest_1.ContextTracker.get()) === null || _a === void 0 ? void 0 : _a.noCache) !== null && _b !== void 0 ? _b : false;
            if (noCache) {
    ...
    async getGroupsByOwner(address) {
        return await this.getData(cache_info_1.CacheInfo.groupsByOwner(address).key, () => this.abiService.getGroupsByOwner(address), cache_info_1.CacheInfo.groupsByOwner(address).ttl);
    }
    ...
    async getEsdtToken(identifier, address) {
        try {
    ...
    async getLockedAssetTokenIdByVestingAddress(vestingAddress) {
        return await this.getData(cache_info_1.CacheInfo.lockedTokenId(vestingAddress).key, async () => await this.abiService.getLockedAssetTokenId(vestingAddress), cache_info_1.CacheInfo.lockedTokenId(vestingAddress).ttl);
    }
    async getFarmStakingGroups() {
        ...
        return results
            .filter((group) => group.childContracts.find((childContract) => childContract.state === staking_1.FarmState.SETUP_COMPLETE))
            .flat();
    }
