...
const farm_address_1 = require("../../../models/staking/farm.address");
...
const unbondFarmToken_model_1 = require("../../../models/staking/unbondFarmToken.model");
...
let StakingService = class StakingService {
    ...
    async getMetaEsdtsDetails(address) {
        ...
        const promises = metaEsdts.map(async (metaEsdt) => {
            ...
            if (stakeDecodedAttributes && stakeDecodedAttributes.length > 0) {
                return new stakeFarmToken_model_1.StakeFarmToken(metaEsdt, stakeDecodedAttributes[0]);
            }
            else {
                ...
                if (unbondDecodedAttributes && unbondDecodedAttributes.length > 0) {
                    return new unbondFarmToken_model_1.UnbondFarmToken(metaEsdt, unbondDecodedAttributes[0]);
                }
                else {
                    return new unbondFarmToken_model_1.UnbondFarmToken(metaEsdt, undefined);
                }
            }
        });
    ...
    getFarmAddresses(farmInfo) {
        ...
        return new farm_address_1.FarmAddress({ unlockedRewardsAddress, lockedRewardsAddress });
    }
