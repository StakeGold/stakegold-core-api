...
let StakingService = class StakingService {
    ...
    async getFarms(address, vmQuery) {
        ...
        await Promise.all(farmStakingGroups.map(async (group) => {
            ...
            const decimals = (_d = (_c = (_b = farms.firstOrUndefined()) === null || _b === void 0 ? void 0 : _b.farmingToken) === null || _c === void 0 ? void 0 : _c.decimals) !== null && _d !== void 0 ? _d : 0;
            groups.push({
    ...
    async handleAddressesByGroupId(farmStakingGroup, metaEsdtsDetails, vmQuery) {
        ...
        const results = await Promise.all(addressesByGroupId.map(async (childContract) => {
            var _a, _b, _c;
            const { farmingTokenId, areRewardsLocked, farmingToken, positions } = await this.getFarmInfo(childContract, metaEsdtsDetails, vmQuery);
            ...
            farm.farmStaking = {
                addressWithLockedRewards: areRewardsLocked
                    ...
                    : (_b = farm.farmStaking) === null || _b === void 0 ? void 0 : _b.addressWithLockedRewards,
                addressWithUnlockedRewards: !areRewardsLocked
                    ? childContract.farmAddress.toString()
                    : (_c = farm.farmStaking) === null || _c === void 0 ? void 0 : _c.addressWithUnlockedRewards,
            };
    ...
    getGroupName(farms) {
        ...
        for (let i = 0; i < farms.length; i++) {
            groupName += (_a = farms[i].farmingToken) === null || _a === void 0 ? void 0 : _a.name;
            if (i !== farms.length - 1) {
                groupName += ' / ';
            }
        }
    ...
    async getGroupTokenSupply(group) {
        ...
        for (const farm of group.farms) {
            ...
            if (farmStaking.addressWithLockedRewards) {
                const farmTotalSupply = await this.stakingGetterService.getFarmTokenSupply(farmStaking.addressWithLockedRewards);
                totalLockedValue = totalLockedValue.plus(new bignumber_js_1.default(farmTotalSupply));
            }
            if (farmStaking.addressWithUnlockedRewards) {
                const farmTotalSupply = await this.stakingGetterService.getFarmTokenSupply(farmStaking.addressWithUnlockedRewards);
                totalLockedValue = totalLockedValue.plus(new bignumber_js_1.default(farmTotalSupply));
            }
        }
