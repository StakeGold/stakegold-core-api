...
let StakingComputeService = class StakingComputeService {
    ...
    async computeAnnualPercentageReward(address) {
        ...
        return this.computeCurrentApr(perBlockRewardAmount, farmTotalSupply, maxAnnualPercentageRewards);
    }
    async computeCurrentApr(perBlockRewardAmount, farmTotalSupply, maxAnnualPercentageRewards) {
    ...
    getAllAprs(group) {
        const apr = group.farms.map((farm) => farm.apr).filter((apr) => apr);
        const lockedApr = group.farms.map((farm) => farm.lockedApr).filter((apr) => apr);
        return [...apr.map((a) => Math.floor(a)), ...lockedApr.map((a) => Math.floor(a))];
    }
    computeLowestApr(group) {
    ...
    computeAccumulatedStakings(group) {
        var _a, _b;
    ...
    computeAccumulatedRewards(group) {
        var _a, _b;
        const positions = group.farms.map((farm) => farm.positions).flat();
        let stakings = new bignumber_js_1.default(0);
        for (let i = 0; i < positions.length; i++) {
    ...
    async getRewardsForPosition(stakeAddress, esdt, vmQuery) {
        var _a, _b;
        if (!(0, stakeFarmToken_model_1.isStakeFarmToken)(esdt) || !esdt.decodedAttributes || !esdt.balance || !esdt.attributes) {
