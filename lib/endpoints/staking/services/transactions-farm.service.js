...
let TransactionsFarmService = class TransactionsFarmService {
    ...
    async stake(sender, groupId, args) {
        ...
        if (collection === undefined || collection === '') {
            return this.contextTransactions.esdtTransfer(contract, args.tokens[0], method, [out_1.BytesValue.fromUTF8(groupId), new out_1.BooleanValue(args.lockRewards)], gasLimit, this.apiConfigService.getChainId());
        }
    ...
    async harvest(sender, args) {
        ...
        for (const token of args.tokens) {
            transactions.push(await this.SftFarmInteraction(sender, { tokens: [token], lockRewards: args.lockRewards }, method, gasLimit, []));
        }
        return transactions;
    }
    ...
    async SftFarmInteraction(sender, stakingArgs, method, gasLimit, args) {
        ...
        if (stakingArgs.tokens.length > 1) {
            return this.contextTransactions.multiESDTNFTTransfer(new out_1.Address(sender), contract, stakingArgs.tokens, method, [], gasLimit, this.apiConfigService.getChainId());
        }
